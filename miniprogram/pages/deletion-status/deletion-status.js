    ...
    wx.cloud.callFunction({ name: 'account', data: { type: 'getDeletionStatus' } })
      .then(res => {
        if (res.result.code === 'SUCCESS') {
          ...
          if (s === 'PENDING' || s === 'PROCESSING' || s === 'RETRYING') {
            that.setData({ status: 'processing', retryCount: res.result.data.retryCount || 0 })
          } else if (s === 'COMPLETED' || s === 'DELETED') {
            that.setData({ status: 'completed' })
          } else {
            that.setData({ status: 'processing' })
          }
        } else if (res.result.code === 'USER_NOT_FOUND') {
          that.setData({ status: 'completed' })
        } else {
          that.setData({ status: 'processing' })
        }
