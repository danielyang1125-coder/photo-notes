...
| DEV-10 引用清理与计数校正 | CODE_COMPLETE | NOT_VERIFIED | 108 个 JS 语法/漂移/禁止模式检查通过；40/40 新增单测通过（18 orphan-cleaner + 22 count-corrector）；重构 cleanup/index.js 行内骨架为 DI 工厂模块（orphan-cleaner.js / count-corrector.js），实现 keyset cursor 分页、持久化 checkpoint、dry-run/apply 双模式、批量查询优化、跨用户隔离和计数 clamp（0～5 / ≥0）；云端验证并入 DEV-13 |
| DEV-11 账号注销 | CODE_COMPLETE | NOT_VERIFIED | 审计检查全部通过（DEV-00～DEV-08 static scope）；313/313 单测通过（新增 29 个 account 测试）；重构 account/index.js 为 handlers.js 工厂模式（DI 注入）；实现 requestDeletion（ACTIVE 校验+精确确认文字+短事务 user→DELETING+task 创建+task_key 唯一幂等）、getDeletionStatus（USER_NOT_FOUND 视为 DELETED）；创建 account-delete-worker.js（4 阶段：STORAGE_CLEANUP/RELATED_DATA_CLEANUP/PRIMARY_DATA_CLEANUP/USER_FINALIZE）；支持批量分页、keyset cursor、租约获取/回收、指数退避重试、MANUAL_REQUIRED 终态、USER_FINALIZE 匿名化回执；cleanup/index.js 新增 deleteTaskWorker 触发器和 dailyCleanup 中账号注销任务处理；云端验证并入 DEV-13 |
| DEV-12 cleanup 与安全收口 | CODE_COMPLETE | NOT_VERIFIED | 117 个 JS 语法/漂移/禁止模式检查通过；363/364 单测通过（新增 51 个 DEV-12 测试：35 task-lease + 16 cleanup-orchestration）；1 个失败为预存在的 sharp 依赖问题（与 DEV-12 无关）；日志安全审计 85 个文件零违规。实现：task-lease.js 共享模块（lease 获取/续租/释放/重试/退避）、refactor photo-delete-worker 和 account-delete-worker 使用共享模块（消除 ~120 行重复代码+新增续租）、cleanup/index.js 增强安全摘要（per-worker 状态/耗时/MANUAL_REQUIRED 计数+countBucket 日志）、backend-task-inspect.js 只读检查脚本、backend-task-retry.js 显式重驱脚本、backend-check.js 安全字段扫描（OPENID/_openid 在响应上下文中）、backend-log-audit.js 日志字段白名单审计、backend-audit.js DEV-12 审计检查；云端双触发器/并发租约/安全日志验证并入 DEV-13 |
| DEV-13 契约切换、验收与发布 | CODE_COMPLETE | NOT_VERIFIED | 118 个 JS 语法/漂移/禁止模式检查通过；371/372 单测通过（1 个预存在 sharp 依赖失败）；8 个新增 DEV-13 测试通过；4 个功能开关（UPLOAD_ATTEMPT_REQUIRED/CURSOR_PAGINATION_REQUIRED/ASYNC_PHOTO_DELETE_ENABLED/PUBLIC_RESOURCE_ERROR_MASKING）在 6 个云函数 index.js 冷启动校验；backend-check.js stale .skip() temporaryAllow 条目清理；backend-audit.js 新增 DEV-13 静态审计（功能开关+API 合约+云验收文档+旧协议扫描+配置模板完整性）；创建 API 合约文档（BACKEND-API-CONTRACT-V1.0.0.md）含完整接口规范+错误码表+前端迁移指南；创建 DEV-13 云验收清单（BACKEND-CLOUD-ACCEPTANCE-DEV-13.md）覆盖 §8 全部发布门禁；重写审计结果文档反映 CODE_COMPLETE 状态；云端验收待 CloudBase 环境执行 |
